...
import play.api.libs.EventSource.{
  EventDataExtractor, EventNameExtractor, EventIdExtractor
}
 ...
 * @param name The name of the event. For Server Sent Events in the browser, 
 * this will end up being the event type, so it will be possible to subscribe 
 * to this event by calling: 
...
object Event {
  ...
  implicit val dataExtractor =
    EventDataExtractor[Event](e => Json.stringify(e.data))
